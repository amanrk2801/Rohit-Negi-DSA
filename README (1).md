...
**Master Theorem** (for recurrences T(n) = aT(n/b) + O(n^d)):
...
| O(2^n) | Exponential | Subsets / power set | Astronomical |
...
- n ≤ 500 → O(n³) is fine
- n ≤ 5,000 → O(n²) is fine
- n ≤ 10^6 → O(n log n) is needed
- n ≤ 10^8 → O(n) is needed
- n > 10^8 → O(log n) or O(1) is needed
...
`newHash = (oldHash - text[i] × base^(m-1)) × base + text[i + m]`
...
- **Perfect Binary Tree** — all internal nodes have 2 children and all leaves are at the same level. Has 2^(h+1) - 1 nodes.
...
**Time Complexities:**
- Subsets: O(2^n) — each element is included or not.
...
**Theory:** Move n disks from source to destination using an auxiliary peg. The rule: only one disk moves at a time, and a larger disk can never sit on a smaller one. Requires 2^n - 1 moves.
...
**Idea:** For n elements, there are 2^n subsets. Each subset maps to a binary number from 0 to 2^n-1. The ith bit being 1 means element i is included.
...
**Theory:** Visit all n cities exactly once and return to start with minimum cost. State: `dp[mask][u]` = min cost to visit cities in `mask` ending at city u. Transition: for each unvisited city v, `dp[mask | (1<<v)][v] = min(dp[mask][u] + dist[u][v])`. Time: O(2^n · n²). Practical for n ≤ 20.
...
- `(a * b) % m = ((a % m) * (b % m)) % m`
- `(a - b) % m = ((a % m) - (b % m) + m) % m`
- Division requires **modular inverse**: `a / b mod m = a * b^(-1) mod m`.
- **Fermat's Little Theorem**: if m is prime, `a^(m-1) ≡ 1 (mod m)`, so `a^(-1) ≡ a^(m-2) (mod m)`.
...
| 10 | **Subsets / Backtracking** | Permutations, combinations, power set | O(2^n) or O(n!) |
